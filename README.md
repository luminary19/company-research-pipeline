...
**What does an engagement cost in tool spend?** Both tools bill by usage, and the spend is dominated by crawl discipline. The two Exa cost traps and the pre-scoring rule in [`playbook/04-tools.md`](playbook/04-tools.md) are the levers, and [`/social-crawl`](.claude/commands/social-crawl.md) presents a budget estimate before every Apify launch.
